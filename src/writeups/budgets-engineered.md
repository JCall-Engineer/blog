...
description: "How I rebuilt my childhood budgeting project as FundOS: a C++20 envelope budgeting app with integer-only money math and a thin, cross-platform GUI."
...
Then a friend asked me to help him build a budget in Excel. That led me to recreate my system in VBA. I used that version for a while, but Excel felt too rigid for what I needed. Around this time, I was also managing my own income as a college student, paid by the hour while juggling school and inconsistent paychecks. Balancing school while managing an hourly income made it increasingly apparent that a budgeting system built around predictable monthly income was a poor fit for many people. Life is messy, and I wanted budgeting software that reflected that reality rather than an idealized one.
...
One function, both phase types, no manual overloading. It was only after I'd leaned on this pattern a few times that I learned generic lambdas actually shipped in C++14, not C++17 or C++20 as I'd assumed while catching up. A small reminder that "new to me" and "new" are not the same thing.
...
It's good to think about what numbers can be represented when you're defining types. On the extreme end, the largest dollar amount FundOS can express for USD, which uses a scale of 100, is $\frac{2^{63}}{100}$ since `int64_t` is a signed integer. That's roughly $92,233 trillion. Even if someone somehow amassed a fortune well over a trillion dollars and converted every cent of it into Iraqi dinar, one of the few currencies using a scale of 1000, the resulting balance would still use only about 15% of the available range. In other words, `int64_t` provides far more headroom than any realistic financial application is likely to need.
...
The check-then-use pattern repeats everywhere `result<T>` appears, and that repetition is intentional. Still, I won't overstate what it buys me. Skipping the check and calling `value()` on an error state throws `std::bad_variant_access`, so if that code path executes, the mistake surfaces. But if it doesn't execute, nothing catches it. A caller who never exercises the failure case in testing can ship code that calls `value()` unchecked and never know it's wrong until it actually fails in production. The type doesn't guarantee correctness, it just makes the correct pattern easy to repeat and the incorrect one occasionally loud.
